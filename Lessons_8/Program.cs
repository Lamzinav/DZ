...
Console.WriteLine($"\nThe program counts the sum of elements in each line and returns the number of the line with the smallest sum of elements: {minSum+1} - line");
*/
...
Console.Write("Enter the number of array columns: ");
int columns = Convert.ToInt32(Console.ReadLine());
Console.Write("Enter the number of columns of the 2nd array:");
...
int[,] second = new int[columns, columns2];
GetArray(second);
Console.WriteLine($"\nSecond array:");
PrintArray(second);
...
ArrayMultiplications(first, second, result);
Console.WriteLine($"\nMultiplication:");
PrintArray(result);

void ArrayMultiplications(int[,] first, int[,] second, int[,] result)
{
  for (int i = 0; i < result.GetLength(0); i++)
  {
    ...
    {
      int sum = 0;
      for (int q = 0; q < first.GetLength(1); q++)
      {
